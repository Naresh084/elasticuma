...
def test_runtime_status_binds_binaries_to_bundled_and_staged_patch(
    ...
    patch = project / "runtime/patches/elasticuma-purgeable.patch"
    source_patch = Path(__file__).resolve().parents[1] / patch.relative_to(project)
    patch.parent.mkdir(parents=True)
    patch.write_bytes(source_patch.read_bytes())
    runtime = tmp_path / "runtime"
    (runtime / ".git").mkdir(parents=True)
    release = runtime / ".build/release"
    release.mkdir(parents=True)
    for name in ("slipstream-repack", "slipstream", "slipstream-server"):
        binary = release / name
        binary.write_bytes(b"binary")
        binary.chmod(0o755)
    ...
    def fake_run(command, **kwargs):
        ...
        return SimpleNamespace(returncode=0, stdout=patch.read_bytes())
