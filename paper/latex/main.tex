...
\begin{center}
  ...
  \vspace{0.12em}
  {\small\href{mailto:naresh@proscendia.com}{naresh@proscendia.com}\par}
  \vspace{0.4em}
...
The source, exact upstream patch, experiment harness, admitted receipts, and
analysis programs are available in the public repository linked on the first
page. The tagged artifact archive includes internal SHA-256 checksums and the
raw evidence used to derive every reported table and quantitative figure. Model
weights, credentials, machine identifiers, and local paths are excluded. Model
checkpoints must be obtained from their original publishers under their own
licenses.
...
Protocols and admission & \code{configs/}, \code{src/elasticuma/admission.py}, and \code{docs/METHODOLOGY.md} \\
Measured evidence & \code{artifacts/releases/} with raw and admitted JSON receipts \\
Analysis and figures & \code{scripts/}, \code{docs/RESULTS.md}, and \code{paper/figures/} \\
Paper source & \code{paper/latex/}, including the bibliography and vector figures \\
\bottomrule
\end{tabularx}
\end{center}

The reconstruction sequence and hardware-reporting fields are documented in
\code{docs/REPRODUCIBILITY.md}. The repository deliberately keeps negative
results and failed alternatives in the same evidence lineage as admitted
results; reproductions that disagree with the reported measurements should be
